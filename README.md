...
```
这类错误,则因为mysql版本高而`mysql-connector-java`版本过低问题，可以将`lib`里的`mysql-connector-java.jar`包换为8.0以上版本，并在`generatorSqlmapCustom.iml`里相应修改配置，同时连接URL改为：`jdbc:mysql://localhost:3306/hrms?serverTimezone=UTC`，此时生成的实体类会出现`WithBLOBs`类，可以查看官方文档解决。
